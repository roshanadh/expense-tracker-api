...
public class CategoryRepositoryImpl implements CategoryRepository {
	...
	private static final String SQL_FIND_BY_ID = "SELECT C.CATEGORY_ID, C" +
			".USER_ID, C.TITLE, C.DESCRIPTION, COALESCE(SUM(T.AMOUNT), 0) " +
			"TOTAL_EXPENSE " +
			"FROM " +
			"ET_TRANSACTIONS" +
			" T RIGHT OUTER JOIN ET_CATEGORIES C ON C.CATEGORY_ID = T" +
			".CATEGORY_ID WHERE C.USER_ID = ? AND C.CATEGORY_ID = ? GROUP BY " +
			"C.CATEGORY_ID";

	private static final String SQL_CREATE = "INSERT INTO ET_CATEGORIES " +
			"(CATEGORY_ID, USER_ID, TITLE, DESCRIPTION) VALUES(NEXTVAL" +
			"('ET_CATEGORIES_SEQ'), ?, ?, ?)";
	...
	public Category findById(Integer userId, Integer categoryId) throws EtResourceNotFoundException {
		try {
			return jdbcTemplate.queryForObject(SQL_FIND_BY_ID,
					new Object[] {userId, categoryId}, categoryRowMapper);
		} catch (Exception e) {
	...
	private RowMapper<Category> categoryRowMapper = ((rs, rowNum) -> {
		...
		return new Category(
				...
				rs.getDouble("TOTAL_EXPENSE") // OR, rs.getDouble(5);
		);
	});
}
